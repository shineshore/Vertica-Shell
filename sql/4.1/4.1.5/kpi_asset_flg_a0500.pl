...
select   Asset_Row_Id
      ...
      FROM  BSSDATA.OFR_ASSET_CDSC_HIST_${LocalCode}       P  
...
SELECT  p.Asset_Integ_Id            ----------alter by chenk 20130121,更改关联字段，ASSET_INTEG_ID-->ASSET_ROW_ID
		...
		FROM  BSSDATA.FIN_PG_OWE_STMT_ITEM_D_${LocalCode} p
	LEFT JOIN  BSSDATA.OFR_MAIN_ASSET_HIST_${LocalCode} P1
...
--                    FROM  BSSDATA.OFR_ASSET_INST_STAT_HIST_${LocalCode}                   
                    FROM  BSSDATA.OFR_ASSET_CDSC_HIST_${LocalCode}                   					
    ...
    FROM
        (
            SELECT
                ...
                END VAL
            FROM
			    BSSDATA.OFR_ASSET_EXI_HIST_${LocalCode} P1
                
            LEFT JOIN
				BSSDATA.OFR_ASSET_EXI_HIST_${LocalCode} P2
...
(select   					P1.ASSET_ROW_ID--V2.7 改用Asset_row_id关联
	   ...
	   FROM   BSSDATA.FIN_PG_OWE_STMT_ITEM_D_${LocalCode}   P1
...
( SELECT ASSET_ROW_ID,BIL_FLG FROM(
	...
	LEFT JOIN   
				BSSDATA.OFR_CHILD_ASSET_HIST_${LocalCode} P2
...
SELECT            P1.Asset_Row_Id
                 ...
                 END AS BIL_FLG  ----v2.0 新计费口径
                   ,(CASE WHEN P1.Stat_Name = '不活动' THEN 0          ---ADD BY ZN20100407
                        ...
                        WHEN CAST(cast(P1.Serv_Start_Dt as date) AS CHAR(10))=CAST(TO_DATE('${TX_DATE}'  ,'YYYYMMDD') AS CHAR(10)) THEN 1
                        ELSE 0
