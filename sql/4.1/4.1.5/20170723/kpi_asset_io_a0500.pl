...
SELECT     P5.Order_Item_Row_Id
	 ...
	 FROM    BSSDATA.EVT_ORDI_HIST_${LocalCode}    P5
...
SELECT     P1.Order_Item_Row_Id
     ...
     FROM    BSSDATA.EVT_ORDI_HIST_${LocalCode}  P1
...
SELECT      ROW_NUMBER() OVER (PARTITION BY Asset_Integ_Id ORDER BY Stat_Name DESC,Start_Dt DESC) AS RANK
	...
	FROM       BSSDATA.OFR_MAIN_ASSET_HIST_${LocalCode}
...
SELECT       P0.ASSET_ROW_ID
		...
		FROM   BSSDATA.OFR_MAIN_ASSET_HIST_${LocalCode}  P0
...
SELECT       P7.Asset_Row_Id
	...
	FROM   BSSDATA.MKT_ASSET_CLAIM_${LocalCode}    P7
LEFT JOIN   BSSDATA.OFR_MKT_CHANNEL_${LocalCode}          P14     
...
SELECT    
   ...
   INNER JOIN     BSSDATA.EVT_ORDER_HIST_${LocalCode}           P1       
...
LEFT JOIN BSSDATA.PAR_CCUST_HIST_${LocalCode} P2
...
FROM 				BSSDATA.OFR_ASSET_EXI_HIST_${LocalCode} P1
LEFT JOIN   BSSDATA.OFR_ASSET_EXI_HIST_${LocalCode} P2
ON					P1.ASSET_ROW_ID = P2.ASSET_ROW_ID
AND         P2.Start_Dt<=TO_DATE('$TX_DATE' ,'YYYYMMDD')
AND         P2.End_Dt>TO_DATE('$TX_DATE'  ,'YYYYMMDD')
...
INSERT INTO KPI.KPI_ASSET_IO_${LocalCode}
  ...
  LEFT join BSSDATA.OFR_ASSET_PROM_INTEG_HIST_${LocalCode}   P2
         ON P1.ASSET_ROW_ID =P2.ASSET_ROW_ID
        and P2.START_DT<=TO_DATE('$TX_DATE'  ,'YYYYMMDD') 
        AND P2.END_DT>TO_DATE('$TX_DATE'  ,'YYYYMMDD') 
...
DELETE FROM KPI.KPI_ASSET_IO_${LocalCode}
WHERE Ordi_Row_Id IN (select ROOT_ORDER_ITEM_ID FROM BSSDATA.EVT_ORDI_HIST_${LocalCode} WHERE ETL_DT = TO_DATE('$TX_DATE' , 'YYYYMMDD')  
