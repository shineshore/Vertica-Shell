...
SELECT     P1.Asset_Row_Id 
      ...
      FROM  BSSDATA_2.OFR_MAIN_ASSET_HIST_${LocalCode}   P1
...
select   Asset_Row_Id
      ...
      FROM  BSSDATA_2.OFR_ASSET_CDSC_HIST_${LocalCode}       P  
...
AS(
    SELECT  Asset_Row_Id
            ...
            ,MAX(CASE WHEN Call_Limit_Stat IN('F0K','F0P')                                         ----F0K（双停）    ----V2.1 
                      ...
                      ELSE cast(Deal_Time as date) 
                  END ) AS Oneway_Stop_Dt
            ,MAX(CASE WHEN Call_Limit_Stat='F0M'                                         ----F0M（单停）
                      THEN TO_DATE('19990101' ,  'YYYYMMDD')
                      ELSE cast(Deal_Time as date) 
                  END ) AS Twoway_Stop_Dt
      FROM  BSSDATA_2.OFR_ASSET_CALL_LIMIT_LOG_${LocalCode}
     WHERE  cast(Deal_Time as date) <=TO_DATE('${TX_DATE}' ,  'YYYYMMDD')
...
AS(
    SELECT  Asset_Row_Id
            ...
            ,MAX(CASE WHEN Call_Limit_Stat IN('F0K','F0P')                                             ----V2.1 
                      THEN TO_DATE('19990101' ,  'YYYYMMDD')
                      ELSE cast(Stat_Upd_Time as date) 
      ...
      FROM  BSSDATA_2.OFR_ASSET_CALL_LIMIT_LOG_${LocalCode}
     WHERE  cast(Stat_Upd_Time as date) <=TO_DATE('${TX_DATE}' ,  'YYYYMMDD')
...
SELECT  p.Asset_Integ_Id            ----------alter by chenk 20130121,更改关联字段，ASSET_INTEG_ID-->ASSET_ROW_ID
            ,p1.ASSET_ROW_ID
            ,SUM(Amt)   Amt
     FROM  BSSDATA_2.FIN_PG_OWE_STMT_ITEM_D_${LocalCode} p
LEFT JOIN  BSSDATA_2.OFR_MAIN_ASSET_HIST_${LocalCode} P1
...
SELECT  P1.*
      ...
      LEFT JOIN   (SELECT  Asset_Row_Id
                    ...
                    FROM  BSSDATA_2.OFR_ASSET_INST_STAT_HIST_${LocalCode}
...
SELECT
						...
						,CASE WHEN (P2.VAL IS NOT NULL AND P2.VAL <> '-1') THEN P2.VAL
									ELSE P1.VAL
						END VAL
FROM 				BSSDATA_2.OFR_ASSET_EXI_HIST_${LocalCode} P1
LEFT JOIN   BSSDATA_2.OFR_ASSET_EXI_HIST_${LocalCode} P2
ON					P1.ASSET_ROW_ID = P2.ASSET_ROW_ID
...
(select   					P1.ASSET_ROW_ID--V2.7 改用Asset_row_id关联
             ...
             FROM   BSSDATA_2.FIN_PG_OWE_STMT_ITEM_D_${LocalCode}   P1
...
CREATE LOCAL TEMP TABLE NEW_BIL_FLG_${LocalCode}_4 ON COMMIT PRESERVE ROWS
AS
(
		SELECT
							P1.ASSET_ROW_ID
...
SELECT ASSET_ROW_ID,BIL_FLG FROM (								  
		SELECT
			...
			FROM		NEW_BIL_FLG_${LocalCode}_4 P1 --??? orignal 1
 LEFT JOIN    BSSDATA_2.OFR_CHILD_ASSET_HIST_${LocalCode} P2
...
SELECT            P1.Asset_Row_Id
                 ...
                 END AS BIL_FLG  ----v2.0 新计费口径
                   ,(CASE WHEN P1.Stat_Name = '不活动' THEN 0          ---ADD BY ZN20100407
                        ...
                        WHEN CAST(cast(P1.Serv_Start_Dt as date) AS CHAR(6))=CAST(TO_DATE('${TX_DATE}' ,  'YYYYMMDD') AS CHAR(6)) THEN 1
                        ELSE 0
      ...
      LEFT JOIN  	 ZJBIC_2.OFR_MAIN_ASSET_FLG_${LocalCode}             			 P16           ---月表
      ...
      LEFT JOIN    ZJBIC_2.NEW_MARKET_AREA_NAME_${LocalCode}  P28
...
DELETE FROM KPI_2.KPI_ASSET_FLG_${LocalCode}_4
WHERE DATE_CD = TO_DATE('$TX_DATE' ,  'YYYYMMDD')
   OR (DATE_CD <= TO_DATE('$TX_DATE' ,  'YYYYMMDD') - 62 )
;   


INSERT INTO KPI_2.KPI_ASSET_FLG_${LocalCode}_4
SELECT
Asset_Row_Id
,Date_Cd
,Bil_Flg
...
CREATE LOCAL TEMP TABLE TMP_OFR_MAIN_ASSET_${LocalCode}_4_TMP ON COMMIT PRESERVE ROWS  --???? Original 1
AS
(
select
P1.ASSET_ROW_ID
...
INNER join KPI_2.KPI_ASSET_FLG_${LocalCode}_4   P3
...
CREATE LOCAL TEMP TABLE TMP_OFR_MAIN_ASSET_${LocalCode}_4 ON COMMIT PRESERVE ROWS
AS
(
select 
P1.ASSET_ROW_ID
...
FROM TMP_OFR_MAIN_ASSET_${LocalCode}_4_TMP   P1
LEFT join BSSDATA_2.OFR_ASSET_CDSC_HIST_${LocalCode}   P2
ON P1.ASSET_ROW_ID = P2.ASSET_ROW_ID
and P2.Start_Dt <= TO_DATE('$TX_DATE'  , 'YYYYMMDD') 
and P2.End_Dt   >  TO_DATE('$TX_DATE'  , 'YYYYMMDD')
...
INNER join TMP_OFR_MAIN_ASSET_${LocalCode}_4  P2
ON P1.ASSET_ROW_ID = P2.ASSET_ROW_ID
and P2.Free_Flg = 0
...
DELETE FROM KPI_2.KPI_ASSET_TER_${LocalCode}_4
WHERE (DATE_CD = TO_DATE('$TX_DATE'  , 'YYYYMMDD') or DATE_CD < TO_DATE('$TX_DATE'  , 'YYYYMMDD') - 62 )
;

INSERT INTO KPI_2.KPI_ASSET_TER_${LocalCode}_4
(
Date_Cd
,Esn_Id
,Esn_New_Dt
...
LEFT join KPI_2.KPI_ASSET_TER_${LocalCode}_4  p3
...
SELECT     P5.Order_Item_Row_Id
     ...
     FROM    BSSDATA_2.EVT_ORDI_HIST_${LocalCode}    P5
...
SELECT     P1.Order_Item_Row_Id
     ...
     FROM    BSSDATA_2.EVT_ORDI_HIST_${LocalCode}  P1
...
SELECT      ROW_NUMBER() OVER (PARTITION BY Asset_Integ_Id ORDER BY Stat_Name DESC,Start_Dt DESC) AS Q_RANK
            ...
            ,Asset_Id
 FROM       BSSDATA_2.OFR_MAIN_ASSET_HIST_${LocalCode}
...
SELECT       P0.ASSET_ROW_ID
            ...
            ,P0.Dept_Row_Id
            ,P1.Dept_Name
     FROM   BSSDATA_2.OFR_MAIN_ASSET_HIST_${LocalCode}  P0 
...
SELECT       P7.Asset_Row_Id
            ...
            ,P7.Start_Dt
            ,P14.Area_Name
     FROM   BSSDATA_2.MKT_ASSET_CLAIM_${LocalCode}    P7
LEFT JOIN   BSSDATA_2.OFR_MKT_CHANNEL_${LocalCode}          P14     
...
-------         FROM     ORDI_${LocalCode}_VALID_CARD_TMP                  P0 
-------    LEFT JOIN     BSSDATA_2.PAR_SALES_EMPLOYEE_HIST               P3
...
SELECT    
   ...
   INNER JOIN     BSSDATA_2.EVT_ORDER_HIST_${LocalCode}           P1       
...
CREATE LOCAL TEMP table ORDI_${LocalCode}_4_PRE3 ON COMMIT PRESERVE ROWS
AS(
SELECT    
                   P.Order_Item_Row_Id
                  ,P.Instant_Flg   
    ...
    --LEFT JOIN     BSSDATA_2.PAR_DEPT                                  P13 
    --       ON     P.Sales_Telecom_Area_ID  = P13.Dept_Row_Id                     
    --LEFT JOIN     ZJBIC_2.ORDER_SALE_NAME_ALL_${LocalCode}            P21                 --YY20101201
...
SELECT    
                  ...
                  ,P.Last_Display_Area_Id
                  ,P.Last_Display_Area_Name             
          FROM    ORDI_${LocalCode}_4_PRE3                  P                                              
    LEFT JOIN     ZJBIC_2.ORDER_SALE_NAME_ALL_${LocalCode}            P21                 --YY20101201
...
LEFT JOIN BSSDATA_2.PAR_CCUST_HIST_${LocalCode} P2
...
LEFT JOIN TMP_SPEED_${LocalCode}_temp P17
       ON P1.ASSET_ROW_ID = P17.ASSET_ROW_ID 
---LEFT join BSSDATA_2.OFR_MKT_CHANNEL_${LocalCode} p2       ---alter by chenk 20140210 Last_Display_Area_Id为空处理，按TELECOM_AREA_ID、LATN_Id的顺序落地
...
DELETE FROM KPI_2.KPI_ASSET_IO_${LocalCode}_4
WHERE  DATE_CD = TO_DATE('$TX_DATE'  , 'YYYYMMDD')
;
...
INSERT INTO KPI_2.KPI_ASSET_IO_${LocalCode}_4
                (
                  Asset_Integ_Id
                 ,Asset_Row_Id
                 ,DATE_CD
  ...
  LEFT join BSSDATA_2.OFR_ASSET_PROM_INTEG_HIST_${LocalCode}   P2
         ON P1.ASSET_ROW_ID =P2.ASSET_ROW_ID
        and P2.START_DT<=TO_DATE('$TX_DATE'  , 'YYYYMMDD') 
        AND P2.END_DT>TO_DATE('$TX_DATE'  , 'YYYYMMDD')         
;
...
DELETE FROM KPI_2.KPI_ASSET_IO_${LocalCode}_4
WHERE Ordi_Row_Id IN (select ROOT_ORDER_ITEM_ID FROM BSSDATA_2.EVT_ORDI_HIST_${LocalCode} WHERE ETL_DT = TO_DATE('$TX_DATE' ,  'YYYYMMDD')  
