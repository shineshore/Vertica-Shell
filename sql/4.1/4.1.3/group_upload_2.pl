...
AS(
    SELECT  P2.ORDER_ITEM_ROW_ID
                            ...
                            ,P2.Prom_Flg
                       ,P2.action_name
       --/* haoyf */FROM    STAGE_1.EVT_ORDI_HIST_${LocalCode}  P2
	   FROM    STAGE_1.EVT_ORDI_HIST_${LocalCode}  P2
...
AS(
 ...
 LEFT JOIN    STAGE_1.EVT_ORDER_HIST_Z  P2
	   ...
	   AND    P2.LATN_ID = $Latn_Id
	 WHERE    2=1
----LEFT JOIN    STAGE_1.PAR_SALES_EMPLOYEE_HIST   P3
...
---- LEFT JOIN    STAGE_1.PAR_SALES_DEPT_HIST   P4                        
...
AS(
        SELECT
			...
			WHERE       2=1
 ----LEFT join        ITFJYFX_1DAPD_FIN_ALWANC_Z    P2
...
----【3.2】数据插入目标表    
--/* haoyf */DELETE FROM ITFJYFX_1.DAPD_PRD_PO_INST_${LocalCode}_2
--/* haoyf */WHERE  DAY_ID =  '${TX_N_DATE}' or DAY_ID = '$Begfore_Day2_Yyyy_Mm_N_Dd'
DELETE FROM ITFJYFX_1.DAPD_PRD_PO_INST_${LocalCode}_2
WHERE  DAY_ID =  '${TX_N_DATE}' or DAY_ID = '$Begfore_Day2_Yyyy_Mm_N_Dd'
;

--/* haoyf*/INSERT INTO ITFJYFX_1.DAPD_PRD_PO_INST_${LocalCode}_2
INSERT INTO ITFJYFX_1.DAPD_PRD_PO_INST_${LocalCode}_2
(
DAY_ID                                                
,LATN_ID                      
,OFFER_INST_ID                
...
,EXT_PROD_OFFER_INST_ID                                     -----------V1.1             
        ...
        SELECT 
                            ...
                            ,EXT_PROD_OFFER_INST_ID                                     -----------V1.1    
          --/* haoyf*/  from        ITFJYFX_1.DAPD_PRD_PO_INST_${LocalCode}_2
		    from        ITFJYFX_1.DAPD_PRD_PO_INST_${LocalCode}_2
         --/* haoyf*/WHERE    DAY_ID = '${Begfore_Day1_Yyyy_Mm_N_Dd}'
		 WHERE    DAY_ID = '${Begfore_Day1_Yyyy_Mm_N_Dd}'
;

--/* haoyf*/MERGE INTO ITFJYFX_1.DAPD_PRD_PO_INST_${LocalCode}_ AS P1
MERGE INTO ITFJYFX_1.DAPD_PRD_PO_INST_${LocalCode}_2 AS P1
USING 
(
SELECT                            
DAY_ID                                                
...
--/* haoyf*/UPDATE ITFJYFX_1.DAPD_PRD_PO_INST_${LocalCode}_2
UPDATE ITFJYFX_1.DAPD_PRD_PO_INST_${LocalCode}_2
...
MERGE INTO 
--/* haoyf*/ITFJYFX_1.DAPD_PRD_PO_INST_${LocalCode}_2 AS P1 
ITFJYFX_1.DAPD_PRD_PO_INST_${LocalCode}_2 AS P1 
USING 
(
SELECT 
