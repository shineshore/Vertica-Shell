...
do   
	...
	for (( j=1;(j <= ($parrel - $running)) && ($invoke <= 30) ;j++ ))
 	do	
		...
		${VSQL} -e -i -v ON_ERROR_STOP=1 -c "select exportdata(*  using parameters path='/data/export/test/1/ZJBIC.OFR_MAIN_ASSET_CUR_A_${invoke}.txt', separator=E'\007'	) over (partition auto) from  ZJBIC.OFR_MAIN_ASSET_CUR_A_${invoke};" > ${AUTO_HOME}/logs/4.1.2_ZJBIC.OFR_MAIN_ASSET_CUR_A_${invoke}.log 2>&1  & 
