...
${VSQL} <<- EOF 2>&1 | tee -a ${logFile}
  ...
  CREATE PROJECTION online_sales.online_sales_fact_normalquery
  (
   online_page_key ENCODING RLE,
   pos_transaction_number ENCODING DELTAVAL
  )
  AS
   SELECT online_sales_fact.online_page_key,
          online_sales_fact.pos_transaction_number
   FROM online_sales.online_sales_fact
   ORDER BY online_sales_fact.online_page_key,
            online_sales_fact.pos_transaction_number
  SEGMENTED BY hash(online_sales_fact.pos_transaction_number) ALL NODES KSAFE;

  select refresh('online_sales.online_sales_fact');
	      ...
	      public void run() {
	        Statement stmt = null;
	          ...
	          for(int i=0; i<this.iCount; i++){
				...
				String query = "select count(*) from online_sales.online_sales_fact_normalquery_b0 where online_page_key = " + id.toString();
	            try {
	              // output result
