...
echo "/opt/vertica/bin/admintools -t db_remove_node --hosts=v004,v005 --database=VMart --noprompts" | tee -a ${logFile}
/opt/vertica/bin/admintools -t db_remove_node --hosts=v004,v005 --database=VMart --password vertica --noprompts 2>&1 <<-EOF 2>&1 | tee -a ${LOGFILE}
	y
EOF
...
echo "data on each node after removing nodes"
$VSQL <<-EOF 2>&1 | tee -a ${logFile}
  select node_name, sum(used_bytes)//1024//1024//1024 as used_gigabytes
     from projection_storage
     group by node_name;
